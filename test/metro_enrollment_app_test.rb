...
class MetroEnrollmentAppTest < Minitest::Test
  ...
  def test_post
    ...
    csv_file = 'test.csv'
    app.any_instance.expects(:parse_csv).with(csv_file).returns(csv_rows)
  ...
  def test_post_csv_errors
    error = 'Bad CSV!'
    csv_file = 'test.csv'
    Resque.expects(:enqueue).never
    app.any_instance.expects(:parse_csv).with(csv_file).raises(CSV::MalformedCSVError, error)

    login
    post '/', {'enrollment-data-file' => {:tempfile => csv_file}, 'enrollment-term-id' => '75'}

    assert_match error, last_request.env['rack.session']['flash'][:danger]
    assert_equal 302, last_response.status
    follow_redirect!
    assert_equal '/', last_request.path
  end
